...
// Define variables for place addition
$originName = '';
$destinationName = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $originName = $_POST['place'];
    $destinationName = $_POST['place'];

    // Insert place into origin table
    $originInsertQuery = "INSERT INTO origin (origin_desc) VALUES ('$originName')";
    $originInsertResult = mysqli_query($conn, $originInsertQuery);

    // Insert place into destination table
    $destinationInsertQuery = "INSERT INTO destination (dest_destination) VALUES ('$destinationName')";
    $destinationInsertResult = mysqli_query($conn, $destinationInsertQuery);

    // Check if place addition was successful
    if ($originInsertResult && $destinationInsertResult) {
        echo '<script>alert("Place added successfully!");</script>';
    } else {
        echo '<script>alert("Error adding place: ' . mysqli_error($conn) . '");</script>';
    }
}
?>
